defmodule Indexer.Block.QitmeerCatchup.BoundIntervalSupervisor do
  @moduledoc """
  Supervises the `Indexer.BlockerFetcher.QitmeerCatchup` with exponential backoff for restarts.
  """
  ...
  alias Indexer.Block.QitmeerCatchup
  ...
  defstruct bound_interval: nil,
            fetcher: %QitmeerCatchup.Fetcher{},
            memory_monitor: nil,
            task: nil
  ...
  @doc """
  Starts supervisor of `Indexer.BlockerFetcher.QitmeerCatchup` and `Indexer.BlockFetcher.Realtime`.
  ...
  defp new(%{block_fetcher: common_block_fetcher} = named_arguments) do
    block_fetcher = %Block.Fetcher{common_block_fetcher | broadcast: :catchup, callback_module: QitmeerCatchup.Fetcher}
  ...
  def handle_call({:get_childspec, :task}, _from, %__MODULE__{fetcher: %QitmeerCatchup.Fetcher{} = catchup} = state) do
    {:reply,
     {:ok,
      %{
        id: :task,
        start: {QitmeerCatchup.Fetcher, :task, [catchup]},
        restart: :transient,
        shutdown: @task_shutdown_timeout,
        type: :worker,
        modules: [QitmeerCatchup.Fetcher]
      }}, state}
  end
  ...
  def handle_call(
        {:restart_child, :task},
        _from,
        %__MODULE__{fetcher: %QitmeerCatchup.Fetcher{} = catchup, task: nil} = state
      ) do
    %Task{pid: pid} =
      task = Task.Supervisor.async_nolink(QitmeerCatchup.TaskSupervisor, QitmeerCatchup.Fetcher, :task, [catchup])
  ...
  def handle_call(:which_children, _from, %__MODULE__{task: task} = state) do
    ...
    {:reply, [{:task, child, :worker, [QitmeerCatchup.Fetcher]}], state}
  end

  @impl GenServer
  def handle_info(:catchup_index, %__MODULE__{fetcher: %QitmeerCatchup.Fetcher{} = catchup} = state) do
    {:noreply,
     %__MODULE__{
       state
       | task: Task.Supervisor.async_nolink(QitmeerCatchup.TaskSupervisor, QitmeerCatchup.Fetcher, :task, [catchup])
  ...
  def handle_info(
    ...
    Logger.error(fn ->
      "QitmeerCatchup index stream exited because the archive node endpoint at #{Keyword.get(options, :url)} is unavailable. Restarting"
    end)
  ...
  def handle_info(
    ...
    Logger.error(fn -> "QitmeerCatchup index stream exited with reason (#{inspect(reason)}). Restarting" end)
