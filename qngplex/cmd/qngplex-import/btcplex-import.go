...
type Tx struct {
	...
	BlockHeight uint     `json:"block_order"`
...
type TxoSpent struct {
	...
	BlockHeight uint32 `json:"block_order"`
...
func main() {
	...
	for {
		...
		if bl.Parent == "" {
			...
			block_order = uint(parentheight + 1)
			conn.Do("HSET", fmt.Sprintf("block:%v:h", bl.Hash), "height", block_order)
			prevheight := block_order - 1
			...
			for {
				...
				for _, cprevhash := range prevs {
					if cprevhash == prevhashtest {
						...
						oblock, _ := qngplex.GetBlockCachedByHash(pool, cprevhash)
		...
		conn.Do("ZADD", fmt.Sprintf("height:%v", block_order), bl.BlockTime, bl.Hash)
		conn.Do("HSET", fmt.Sprintf("block:%v:h", bl.Hash), "parent", bl.Parent)

		if latestheight != 0 && !(latestheight+1 <= int(block_order)) {
			...
			if !(len(tx.TxIns) == 1 && tx.TxIns[0].InputVout == 0xffffffff) {
				...
				for txi_index, txi := range tx.TxIns {
					...
					go func(txi *blkparser.TxIn, bl *blkparser.Block, tx *blkparser.Tx, pool *redis.Pool, total_tx_in *uint64, txi_index int) {
						...
						txospent := new(TxoSpent)
						txospent.Spent = true
						txospent.BlockHeight = uint32(block_order)
		...
		conn.Do("MSET", fmt.Sprintf("block:%v", block.Hash), blockjson, "height:latest", int(block_order), fmt.Sprintf("block:height:%v", block.Height), block.Hash)
		block.Txs = txs
		blockjsoncache, _ := json.Marshal(block)
		conn.Do("SET", fmt.Sprintf("block:%v:cached", block.Hash), blockjsoncache)

		if !running {
			log.Printf("Done. Stopped at height: %v.", block_order)
		}
