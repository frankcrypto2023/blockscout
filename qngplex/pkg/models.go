...
func GetBlockCachedByHash(rpool *redis.Pool, hash string) (block *dbmodels.UTXOBlock, err error) {
	GetDB().Where("hash = ?", hash).First(&block)
...
func GetTx(rpool *redis.Pool, hash string) (tx *Tx, err error) {
	var txs []*dbmodels.UTXOTx
	GetDB().Where("txid = ?", hash).Find(&txs)
	if len(txs) == 0 {
	...
	for _, out := range txs {
		outs = append(outs, &TxOut{
			Addr:  out.Address,
			Value: uint64(out.Amount),
			Spent: &TxoSpent{
				Spent: out.SpentStatus == dbmodels.SpentStatusSpent,
			},
	...
	tx = &Tx{
		Hash:        hash,
		BlockHeight: uint(txs[0].BlockOrder),
...
func GetLastXBlocks(rpool *redis.Pool, start uint, stop uint) (blocks []*dbmodels.UTXOBlock, err error) {
	blocks = []*dbmodels.UTXOBlock{}
	GetDB().Debug().Where("block_order >= ? and block_order <= ?", stop, start).Order("block_order desc").Find(&blocks)

	return
}

func GetTxsByBlock(rpool *redis.Pool, block string) (txs []*Tx, err error) {
	dbtxs := []*dbmodels.UTXOTx{}
	// GetDB().Debug().Select("txid").Group("txid").Find(&dbtxs)
	// for _, v := range dbtxs {
	// 	fmt.Println(v.Txid)
	// }
	GetDB().Debug().Select("txid").Where("block = ?", block).Group("txid").Find(&dbtxs)

	txs = []*Tx{}
	for _, dbtx := range dbtxs {
		tx, _ := GetTx(rpool, dbtx.Txid)
		if tx != nil {
			txs = append(txs, tx)
		}
	}
...
type GetBlockOrderResult struct {
	Hash          string              `json:"hash"`
	...
	Transactions  []qjson.TxRawResult `json:"transactions"`
