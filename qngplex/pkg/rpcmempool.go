...
func ProcessUnconfirmedTxs(conf *Config, pool *redis.Pool, running *bool) {
	...
	oldkeys, _ := redis.Strings(c.Do("ZRANGE", "qngplex:rawmempool", 0, -1))
	c.Do("DEL", redis.Args{}.AddFlat(oldkeys)...)
	c.Do("DEL", "qngplex:rawmempool")
	...
	for {
		...
		ckey = fmt.Sprintf("qngplex:rawmempool:%v", cts)
		...
		for _, txid := range unconfirmedtxs {
			...
			go func(pool *redis.Pool, txid string, unconfirmedtxsverbose *map[string]interface{}, ckey string) {
				...
				defer wg.Done()
				defer func() { <-sem }()
				txkey := fmt.Sprintf("qngplex:utx:%v", txid)
				...
				if txmetafound {
					...
					if !txexists {
						tx, _ := GetTxRPC(conf, txid)
						txjson, _ := json.Marshal(tx)
						c.Do("SET", txkey, string(txjson))
					}
					c.Do("ZADD", "qngplex:rawmempool", fseentime, txkey)
		...
		if lastkey != "" {
			...
			c.Do("ZREM", redis.Args{}.Add("qngplex:rawmempool").AddFlat(dkeys)...)
			c.Do("DEL", lastkey)
			// Since getrawmempool return transaction sorted by name, we replay them sorted by time asc
			newkeys, _ := redis.Strings(c.Do("ZRANGEBYSCORE", "qngplex:rawmempool", fmt.Sprintf("(%v", lastts), cts))
			for _, newkey := range newkeys {
				txjson, _ := redis.String(c.Do("GET", newkey))
				ctx := new(Tx)
				json.Unmarshal([]byte(txjson), ctx)
				// Notify SSE unconfirmed transactions
				alreadypublished, _ := redis.Bool(c.Do("EXISTS", fmt.Sprintf("qngplex:utx:%v:published", ctx.Hash)))
				if !alreadypublished {
					c.Do("PUBLISH", "qngplex:utxs", txjson)
					// Notify transaction to every channel address
					multiPublishScript.Do(c, redis.Args{}.Add(txjson).AddFlat(ctx.AddressesChannels())...)
					c.Do("SETEX", fmt.Sprintf("qngplex:utx:%v:published", ctx.Hash), 3600*20, cts)
					//c.Do("SADD", "qngplex:utxs:published", ctx.Hash)
				}
...
func GetUnconfirmedTx(pool *redis.Pool, hash string) (tx *Tx, err error) {
	c := pool.Get()
	defer c.Close()
	txkey := fmt.Sprintf("qngplex:utx:%v", hash)
	tx = new(Tx)
